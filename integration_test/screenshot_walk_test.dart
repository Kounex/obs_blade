...
void main() {
  ...
  testWidgets('visual QA screenshot walk', (WidgetTester tester) async {
    app.main();
    ...
    if (find.byType(IntroView).evaluate().isNotEmpty) {
      // ignore: avoid_print
      print(
        'STATE-INFO: app booted into intro (fresh install) - '
        'replacing root route with /tabs',
      );
      final BuildContext bootIntroContext = tester.element(
    ...
    final Map<dynamic, dynamic> settingsSnapshot = Map<dynamic, dynamic>.from(
      settingsBox.toMap(),
    );
    final bool hadPreviewWarningKey = settingsBox.containsKey(
      SettingsKeys.DontShowPreviewWarning.name,
    );
    final int savedConnectionsCount = Hive.box<Connection>(
      HiveKeys.SavedConnections.name,
    ).length;
    final int customThemeCount = Hive.box<CustomTheme>(
    ...
    final ScrollController? homeScroll = _routeScrollController(
      tester,
    ...
    if (allStats.isNotEmpty) {
      allStats.sort(
        (a, b) => a.listEntryDateMS.last.compareTo(b.listEntryDateMS.last),
      );
      _pushInTab(
        Tabs.Statistics,
    ...
    final ScrollController? settingsScroll = _routeScrollController(
      tester,
    ...
    await _shot(tester, '22_settings_dashboard_customisation', settleMs: 2200);
    _pushInTab(
      Tabs.Settings,
    ...
    final BuildContext settingsContext = tester.element(
      find.byType(SettingsView).first,
    );
    Navigator.of(
      settingsContext,
      rootNavigator: true,
    ...
    if (formFields.evaluate().length < 3) {
      ...
      if (await _tryTap(
        ...
        await ack.future.timeout(const Duration(seconds: 5), onTimeout: () {});
    ...
    if (!dashboardReached) {
      ...
      if (find.text('Save Connection').evaluate().isNotEmpty) {
        await _shot(
          tester,
          '51_dashboard_save_connection_dialog',
          settleMs: 600,
        );
        await _tryTap(
          tester,
      ...
      final ScrollController? dashScroll = _routeScrollController(
        tester,
      ...
      await _scrollUntilHittable(
        tester,
        find.widgetWithText(Tab, 'Scene Items'),
      ...
      if (previewTileTapped) {
        ...
        if (dashboardStore.scenePreviewImageBytes != null &&
          ...
          for (int i = 0; i < 6; i++) {
            final Rect previewRect = tester.getRect(find.byType(ScenePreview));
          ...
          if (find.byIcon(CupertinoIcons.clear).evaluate().isNotEmpty) {
            await _shot(
              tester,
              '60_dashboard_preview_fullscreen',
              settleMs: 700,
            );
            await _tryTap(
              tester,
    ...
    final Map<dynamic, dynamic> settingsAfter = Map<dynamic, dynamic>.from(
      settingsBox.toMap(),
    );
    ...
    final int savedConnectionsAfter = Hive.box<Connection>(
      HiveKeys.SavedConnections.name,
    ).length;
    final int customThemesAfter = Hive.box<CustomTheme>(
      HiveKeys.CustomTheme.name,
    ).length;
