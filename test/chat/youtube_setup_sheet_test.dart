...
void main() {
  ...
  testWidgets('prefills the fields from settings', (tester) async {
    ...
    await tester.runAsync(() async {
      ...
      await settingsBox().put(
        SettingsKeys.YouTubeOAuthClientId.name,
        'saved-client',
      );
    });
  ...
  testWidgets('invalid key shows the inline failure', (tester) async {
    ...
    await tester.enterText(find.byType(NativeChatTextField).first, 'bad-key');
    await tester.pump();
    await tester.tap(find.byKey(const Key('youtube-setup-test-key')));
    await tester.pump();
  ...
  testWidgets('valid key shows the inline success and save persists the keys', (
    ...
    try {
      ...
      await tester.enterText(
        find.byType(NativeChatTextField).first,
        'good-key',
      );
      await tester.pump();
      await tester.tap(find.byKey(const Key('youtube-setup-test-key')));
      ...
      expect(chatService.resolveCalls, 1);
      expect(find.byKey(const Key('youtube-setup-key-valid')), findsOneWidget);
      expect(find.byKey(const Key('youtube-setup-key-invalid')), findsNothing);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      try {
        ...
        await tester.enterText(
          find.byType(NativeChatTextField).at(0),
          'fresh-key',
        );
        await tester.pump();
        await tester.tap(find.byKey(const Key('youtube-setup-test-key')));
        await tester.pump();
        expect(
          find.byKey(const Key('youtube-setup-key-valid')),
          findsOneWidget,
        );
        ...
        await tester.enterText(
          find.byType(NativeChatTextField).at(1),
          'fresh-client',
        );
        await tester.enterText(
          find.byType(NativeChatTextField).at(2),
          'fresh-secret',
        );
        await tester.pump();
        ...
        await tester.ensureVisible(
          find.byKey(const Key('youtube-setup-sign-in')),
        );
        await tester.pump();
        await tester.tap(find.byKey(const Key('youtube-setup-sign-in')));
        await tester.pump();

        expect(settingsBox().get(SettingsKeys.YouTubeApiKey.name), 'fresh-key');
        expect(
