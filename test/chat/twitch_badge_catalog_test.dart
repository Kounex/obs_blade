...
void main() {
  group('TwitchBadgeSet', () {
    test('parses a badge set with versions (helix shape)', () {
      ...
      expect(
        set.versions.single.imageUrl2x,
  ...
  group('settingsKeyForBadgeSetId', () {
    test('maps the dedicated categories', () {
      expect(
        settingsKeyForBadgeSetId('broadcaster'),
        SettingsKeys.TwitchChatBadgeBroadcaster,
      );
      expect(
        settingsKeyForBadgeSetId('moderator'),
        SettingsKeys.TwitchChatBadgeModerator,
      );
      expect(settingsKeyForBadgeSetId('vip'), SettingsKeys.TwitchChatBadgeVip);
      expect(
        settingsKeyForBadgeSetId('subscriber'),
        SettingsKeys.TwitchChatBadgeSubscriber,
      );
      expect(
        settingsKeyForBadgeSetId('founder'),
        SettingsKeys.TwitchChatBadgeFounder,
      );
      expect(
        settingsKeyForBadgeSetId('bits'),
    ...
    test('unknown set ids fall under Other', () {
      for (final setId in [
        ...
        expect(
          settingsKeyForBadgeSetId(setId),
    ...
    test('badge toggle keys have kebab-case names', () {
      expect(
        SettingsKeys.TwitchChatBadgeBroadcaster.name,
        'twitch-chat-badge-broadcaster',
      );
      expect(
        SettingsKeys.TwitchChatBadgeModerator.name,
        'twitch-chat-badge-moderator',
      );
      expect(SettingsKeys.TwitchChatBadgeVip.name, 'twitch-chat-badge-vip');
      expect(
        SettingsKeys.TwitchChatBadgeSubscriber.name,
        'twitch-chat-badge-subscriber',
      );
      expect(
        SettingsKeys.TwitchChatBadgeFounder.name,
        'twitch-chat-badge-founder',
      );
      expect(SettingsKeys.TwitchChatBadgeBits.name, 'twitch-chat-badge-bits');
      expect(SettingsKeys.TwitchChatBadgeOther.name, 'twitch-chat-badge-other');
    });
  });
}
