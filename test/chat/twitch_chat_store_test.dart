...
void main() {
  ...
  group('sendChatMessage', () {
    ...
    test('captures channel and reply before awaiting the access token', () async {
      await login();
    ...
    test('completion does not clear a newer reply in the same channel', () async {
      await login();
      messageService.sendGate = Completer<TwitchSendResult>();
      store.setReplyTarget(chatMessage('original-reply', 'u7'));
      ...
      final nextReply = chatMessage('next-reply', 'u8');
      store.setReplyTarget(nextReply);
      messageService.sendGate!.complete(const TwitchSendResult(messageId: 'sent', isSent: true));
      expect(await pending, isTrue);
      expect(store.replyTarget, same(nextReply));
    });
    ...
    test('send failure does not belong to a newly selected channel', () async {
      ...
      await Future<void>.delayed(Duration.zero);
      store.selectedChannelId = 'other-channel';
      messageService.sendGate!.complete(const TwitchSendResult(messageId: '', isSent: false));
