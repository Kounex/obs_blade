...
void main() {
  ...
  setUpAll(() {
    ...
    expect(
      fixtureBoxes.listSync().whereType<File>().any(
  ...
  test(
    'committed connection + theme + settings values survive cold open',
    () async {
      await harness.openAllBoxes();
  ...
  test('second reopen of the same committed files still works', () async {
    ...
    expect(
      Hive.box<Connection>(
        HiveKeys.SavedConnections.name,
      ).values.any((c) => c.name == 'mDNS host obs-main.local'),
      isTrue,
    );
  });
}
