...
void main() {
  final classicBoxes = Directory('test/persistence/fixtures/classic_boxes');
  ...
  setUpAll(() {
    ...
    expect(
      classicBoxes.listSync().whereType<File>().any(
        (f) => f.path.endsWith('.hive'),
      ),
      isTrue,
    );
  });
  ...
  test(
    'classic-written boxes open under Hive CE with foundation counts',
    () async {
  ...
  test('key foundation rows survive classic → CE open', () async {
    ...
    final theme = Hive.box<CustomTheme>(
    ...
    final raid = Hive.box<PastStreamData>(
  ...
  test('classic → CE boxes still open after a second cold reopen', () async {
    ...
    expect(
      Hive.box<Connection>(
        HiveKeys.SavedConnections.name,
      ).values.any((c) => c.name == 'Living Room PC'),
      isTrue,
    );
  });
}
