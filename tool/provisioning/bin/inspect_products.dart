...
Future<void> main() async {
  ...
  /// Prints "priced/total territories" and nominal-parity mismatches.
  ...
  for (final productId in ['pro_yearly', 'pro_monthly']) {
    ...
    final locs = await asc.get('v1/subscriptions/$subId/subscriptionLocalizations');
    for (final loc in locs.dataList) {
      final a = loc['attributes'] as Map<String, Object?>;
      print('  loc ${a['locale']}: name="${a['name']}" '
  ...
  if (iapFound.dataList.isEmpty) {
    ...
    final locs =
        await asc.get('v2/inAppPurchases/$iapId/inAppPurchaseLocalizations');
    for (final loc in locs.dataList) {
      final a = loc['attributes'] as Map<String, Object?>;
      print('  loc ${a['locale']}: name="${a['name']}" '
          'description="${a['description']}"');
    ...
    if (manual.isNotEmpty) {
      ...
      print('  price: base territory USA set; 178 auto-equalized territory '
  ...
  try {
    ...
    print('Play inspection failed (known: service account currently lacks '
        'read permission): $e');
  }
}
...
Future<void> _inspectPlay(Map<String, String> env) async {
  ...
  for (final l in (subJson['listings'] as List? ?? [])) {
    print('  listing ${l['languageCode']}: title="${l['title']}" '
  ...
  for (final bp in (subJson['basePlans'] as List? ?? [])) {
    final regional = (bp['regionalConfigs'] as List? ?? [])
        ...
        .join(', ');
    print('  basePlan ${bp['basePlanId']} (${bp['state']}) '
