...
class SocialBlock extends StatelessWidget {
  ...
  Future<void> _handleSocialTap(
    ...
    try {
      if (social.deepLink != null &&
          await canLaunchUrl(Uri.parse(social.deepLink!))) {
        if (!await launchUrl(
          Uri.parse(social.deepLink!),
          mode: LaunchMode.externalApplication,
        )) {
          await launchUrl(Uri.parse(social.deepLink!));
        }
      } else if (await canLaunchUrl(Uri.parse(social.link))) {
        if (!await launchUrl(
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Wrap(spacing: 12.0, runSpacing: 12.0, children: socialEntries),
    );
  }
}
