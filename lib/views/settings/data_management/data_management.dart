...
Future<void> deleteAllUserDataPreservingEntitlements() async {
  ...
  bool boughtPro = Hive.box(
    HiveKeys.Settings.name,
  ).get(SettingsKeys.BoughtPro.name, defaultValue: false);
  await Hive.box(HiveKeys.Settings.name).clear();

  Hive.box(
    HiveKeys.Settings.name,
  ).put(SettingsKeys.BoughtBlacksmith.name, boughtBlacksmith);
  Hive.box(HiveKeys.Settings.name).put(SettingsKeys.BoughtPro.name, boughtPro);
}
            ...
            child: DataBlock(
              ...
              dataEntries: [
                ...
                DataEntry(
                  ...
                  onClear: () {
                    for (SettingsKeys key in SettingsKeys.values.where(
                      (key) => key.name.startsWith('dont-show'),
                    )) {
